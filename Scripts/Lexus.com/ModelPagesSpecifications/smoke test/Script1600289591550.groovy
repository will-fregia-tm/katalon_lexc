...
previousFeedData = findTestData(GlobalVariable.DS_version + 'Log').getValue(1, 21)
...
if (WebUI.verifyNotMatch(currentFeedData, previousFeedData, false, FailureHandling.OPTIONAL)) {
    ...
    for (int rowIndex = 2; rowIndex < 5; rowIndex++) {
        ExcelKeywords.setValueToCellByIndex(sheet01, 21, 0, textToWrite)
    }
    ...
    for (def index : (0..totalPages)) {
        WebUI.navigateToUrl(findTestData(GlobalVariable.DS_version + 'URLsModelPagesSpecifications').getValue(dataColumn, dataRow))
