...
previousFeedData = findTestData(GlobalVariable.DS_version + 'Log').getValue(1, 11)
...
if (WebUI.verifyNotMatch(currentFeedData, previousFeedData, false, FailureHandling.OPTIONAL)) {
    ...
    for (int rowIndex = 2; rowIndex < 5; rowIndex++) {
        ExcelKeywords.setValueToCellByIndex(sheet01, 11, 0, textToWrite)
    }
    
    ExcelKeywords.saveWorkbook(excelFilePath, workbook01)

    'counts the total number of URLs to be tested'
    totalPages = (findTestData(GlobalVariable.DS_version + 'URLsModelPagesAccessories').getRowNumbers() - 1)

    WebUI.openBrowser(GlobalVariable.TS_Domain + GlobalVariable.Header)

    for (def index : (0..totalPages)) {
        WebUI.navigateToUrl(findTestData(GlobalVariable.DS_version + 'URLsModelPagesAccessories').getValue(dataColumn, dataRow))
