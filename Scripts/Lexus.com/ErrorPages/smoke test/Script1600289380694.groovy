...
previousFeedData = findTestData(GlobalVariable.DS_version + 'Log').getValue(1, 6)
...
if (WebUI.verifyNotMatch(currentFeedData, previousFeedData, false, FailureHandling.OPTIONAL)) {
    ...
    for (int rowIndex = 2; rowIndex < 5; rowIndex++) {
        ExcelKeywords.setValueToCellByIndex(sheet01, 6, 0, textToWrite)
    }
    
    ExcelKeywords.saveWorkbook(excelFilePath, workbook01)

    'counts the total number of URLs to be tested'
    totalPages = (findTestData(GlobalVariable.DS_version + 'URLsErrorPages').getRowNumbers() - 1)

    WebUI.openBrowser(GlobalVariable.TS_Domain + GlobalVariable.Header)

    for (def index : (0..totalPages)) {
        WebUI.navigateToUrl(findTestData(GlobalVariable.DS_version + 'URLsErrorePages').getValue(dataColumn, dataRow))
        ...
        WebUI.verifyElementPresent(findTestObject('error'), 0)
