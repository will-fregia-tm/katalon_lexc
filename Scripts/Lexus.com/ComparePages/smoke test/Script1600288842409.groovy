...
previousFeedData = findTestData(GlobalVariable.DS_version + 'Log').getValue(1, 4)
...
if (WebUI.verifyNotMatch(currentFeedData, previousFeedData, false, FailureHandling.OPTIONAL)) {
    ...
    for (int rowIndex = 2; rowIndex < 5; rowIndex++) {
        ExcelKeywords.setValueToCellByIndex(sheet01, 4, 0, textToWrite)
    }
    
    ExcelKeywords.saveWorkbook(excelFilePath, workbook01)

    'counts the total number of URLs to be tested'
    totalPages = (findTestData(GlobalVariable.DS_version + 'URLsComparePages').getRowNumbers() - 1)

    WebUI.openBrowser(GlobalVariable.TS_Domain + GlobalVariable.Header)

    for (def index : (0..totalPages)) {
        WebUI.navigateToUrl(findTestData(GlobalVariable.DS_version + 'URLsComparePages').getValue(dataColumn, dataRow))
