...
if (WebUI.verifyElementPresent(findTestObject('Homepage/HeroOffers/zip code field'), 5, FailureHandling.OPTIONAL)) {
    ...
    actualValue = WebUI.getText(findTestObject('Homepage/HeroOffers/OfferRow/offer card 1 - model-year'))

    expectedValue = findTestData('HP').getValue(column, 26)

    valueWithoutExpected = (actualValue - expectedValue)

    'model series-year in offer card should match expected result from datasheet'
    WebUI.verifyNotMatch(valueWithoutExpected, actualValue, false, FailureHandling.STOP_ON_FAILURE)

    'these steps validate that the actual value contains an expected value'
    actualValue = WebUI.getText(findTestObject('Homepage/HeroOffers/OfferRow/offer card 2 - model-year'))

    expectedValue = findTestData('HP').getValue(column, 27)
    ...
    'these steps validate that the actual value contains an expected value'
    actualValue = WebUI.getText(findTestObject('Homepage/HeroOffers/OfferRow/offer card 3 - model-year'))

    expectedValue = findTestData('HP').getValue(column, 28)
