...
WebUI.navigateToUrl((GlobalVariable.AEM_Domain + findTestData('modelData').getValue(GlobalVariable.dataColumn, seriesKey)) + 
    GlobalVariable.deeplinkPerformance)

WebUI.waitForElementPresent(findTestObject('ModelPages/Performance/performance module'), 10, FailureHandling.OPTIONAL)

WebUI.delay(4)

'Visiting via deeplink scrolls to predetermined location, with necessary elements preselected/shown/active.'
WebUI.verifyElementPresent(findTestObject('ModelPages/Performance/performance module'), 5, FailureHandling.STOP_ON_FAILURE)
...
if (WebUI.verifyMatch(GlobalVariable.contentValidation, 'yes', false, FailureHandling.OPTIONAL)) {
    'Visiting via deeplink scrolls to predetermined location, with necessary elements preselected/shown/active.'
    WebUI.verifyElementVisibleInViewport(findTestObject('ModelPages/Performance/performance module'), 5, FailureHandling.STOP_ON_FAILURE)
}
