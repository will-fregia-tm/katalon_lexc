...
if (WebUI.verifyMatch(GlobalVariable.author, 'yes', false, FailureHandling.OPTIONAL)) {
    WebUI.navigateToUrl(((GlobalVariable.AEM_Domain + findTestData('modelData').getValue(GlobalVariable.dataColumn, seriesKey)) + 
        GlobalVariable.authorQuery) + GlobalVariable.deeplinkStyles)
}

WebUI.waitForElementPresent(findTestObject('ModelPages/Styles/styles module'), 10, FailureHandling.OPTIONAL)

WebUI.delay(4)

'Visiting via deeplink scrolls to predetermined location, with necessary elements preselected/shown/active.'
WebUI.verifyElementPresent(findTestObject('ModelPages/Styles/styles module'), 5, FailureHandling.STOP_ON_FAILURE)
...
if (WebUI.verifyMatch(GlobalVariable.contentValidation, 'yes', false, FailureHandling.OPTIONAL)) {
    'Visiting via deeplink scrolls to predetermined location, with necessary elements preselected/shown/active.'
    WebUI.verifyElementVisibleInViewport(findTestObject('ModelPages/Styles/styles module'), 5, FailureHandling.STOP_ON_FAILURE)
}
